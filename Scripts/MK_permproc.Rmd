...
Permutation procedure (Type 1) - shuffle SOURCE and TARGET labels non-independently (i.e. SOURCE labels can become TARGET labels and vice versa)
...
ggplot(plot_df,aes(PNC)) + geom_histogram(col="gray",fill="black",bins=10) + theme_classic() + xlab("Percentage of 'non-events' with the same \n labels as their corresponding events \n following permutation") + ylab("Frequency") + theme(text=element_text(family="sans",size=11)) #Plot histogram of percentage of 'non-events' that retain the same source and target labels as their corresponding observed events following permutation.
```




Permutation procedure (Type 2) - shuffle either SOURCE or TARGET labels independently
...
Permutation procedure (Type 3) - pairwise (i.e. row-wise) swaps of source and target labels. 
```{r}
...
Permutation procedure (Type 4) - shuffle edge weights or arrival latencies only (i.e. without altering the underlying network structure)
